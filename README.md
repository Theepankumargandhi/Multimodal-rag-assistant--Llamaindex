...
```
docker-compose up -d  # Start all services (Streamlit app + Neo4j)
...
Neo4j Browser: http://localhost:7474 (user: neo4j, password: as configured in docker-compose.yml)
...
```
# Pull the image
docker pull ghcr.io/theepankumargandhi/multimodal-rag-assistant:latest
...
docker run -p 8501:8501 \
  ...
  ghcr.io/theepankumargandhi/multimodal-rag-assistant:latest
```
...
Check the build status in the Actions tab
---
...
* Top-K settings for result count
* Graph-RAG parameters (confidence thresholds, hop limits)
